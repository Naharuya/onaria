...
test('all religions and every fallback entry preserve crisis with zero downstream calls', async () => {
  ...
  const service = createConversationService({ env: { ONARIA_COST_ROUTER_V1_ENABLED: 'false', ONARIA_AI_MODE: 'openai', ONARIA_MULTI_AGENT_ENABLED: 'true', OPENAI_API_KEY: 'test-only' },
