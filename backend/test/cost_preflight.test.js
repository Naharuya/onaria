...
const env = { OPENAI_API_KEY: 'sk-synthetic-preflight-fixture-only', ONARIA_AI_MODE: 'openai', ONARIA_MULTI_AGENT_ENABLED: 'true' };
...
test('provider configuration rejects absent/invalid key, disabled provider and unknown prices before tests or payment', async () => {
  for (const override of [{ OPENAI_API_KEY: '' }, { OPENAI_API_KEY: 'invalid' }, { ONARIA_AI_MODE: 'local' },
    { ONARIA_EXTERNAL_API_DISABLED: 'true' }, { OPENAI_SOL_MODEL: 'unpriced-fixture' }]) {
    let calls = 0;
