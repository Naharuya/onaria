...
test('specialization: false flag keeps preexisting local behavior for every question', async () => {
  ...
  const off = createConversationService({ env: { ONARIA_COST_ROUTER_V1_ENABLED: 'false', ONARIA_AI_MODE: 'openai', ONARIA_MULTI_AGENT_ENABLED: 'false' }, logger, openAiFactory: () => assert.fail('disabled') });
...
test('specialization: inferred non-Christian provider failure never offers Bible verses', async () => {
  const generate = createConversationService({ env: { ONARIA_COST_ROUTER_V1_ENABLED: 'false', ONARIA_AI_MODE: 'openai', ONARIA_MULTI_AGENT_ENABLED: 'true', OPENAI_API_KEY: 'test-placeholder' }, logger: { info() {}, warn() {} }, openAiFactory: () => { throw Error('offline'); } });
