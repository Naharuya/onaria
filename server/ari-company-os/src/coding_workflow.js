...
export class CodingWorkflow{
 constructor({stateRoot,worker,repairWorker,osRoot}){this.stateRoot=stateRoot;this.osRoot=osRoot;this.worker=worker??createCodexRepairWorker({enabled:process.env.ARI_CODEX_CODING_ENABLED==='true',mode:'coding'});this.repairWorker=repairWorker??createCodexRepairWorker({enabled:process.env.ARI_CODEX_REPAIR_ENABLED==='true'});}
 async execute(task,{repair=false}={}){
  ...
  try{
   ...
   for(const file of files){
    ...
    // Preserve the triggering coding diff for a repair; never copy unrelated files or credentials.
   ...
   let specialist;
   if(task.owner==='rami-audio-manager')specialist=fs.readFileSync(path.resolve(this.osRoot,'../../.github/agents/rami-audio-manager.agent.md'),'utf8');
   const worker=repair?this.repairWorker:this.worker;
   const result=await worker({cwd,branch,project:task.project,failedCheck,result:task.failure??{reason:'approved_development'},attempt:(task.repairAttempts??0)+1,context:{acceptanceCriteria:task.acceptanceCriteria,specialist}});
   ...
   const focused=[];
   if(!repair){
    if(!task.focusedChecks?.length)return review('focused_test_required');
    for(const check of task.focusedChecks){
     const checkDir=path.resolve(cwd,check.cwd??'.');if(checkDir!==cwd&&!checkDir.startsWith(cwd+path.sep))return review('focused_test_outside_worktree');
     if(!['node','npm','flutter',process.execPath].includes(check.command)||/deploy|publish|release|live_trade/.test((check.args??[]).join(' ')))return review('focused_command_blocked');
     const r=await runCommand(check.command,check.args??[],{cwd:checkDir,env:check.env,timeoutMs:check.timeoutMs});focused.push({id:check.id,...r});if(!r.ok)return {state:'FAIL',reason:'focused_test_failed',worktree:cwd,focused};
    }
   }
   const diff=git(cwd,['diff','--no-ext-diff','--',...files]);
   const dir=path.join(this.stateRoot,'reports',task.id);fs.mkdirSync(dir,{recursive:true});const diffPath=path.join(dir,'changes.diff');fs.writeFileSync(diffPath,diff+'\n',{flag:'wx',mode:0o600});
