...
2026-09-13 공개 운영 홈페이지 `https://onaria.ai.kr`를 읽기 전용으로 확인했다(HTTP 200). 기존 페이지는 8개 섹션이며 ONARIA 이름과 Organization 구조화 데이터가 없었다. 운영 서버 설정이나 데이터는 변경하지 않았다.
...
Hero → Why ONARIA → How it works → AI 마음대화 → 7 Paths → ONARIA → Safety & Trust → Privacy → Ecosystem → Final CTA → Footer.
...
- 일곱 전통의 카드 크기·색·상태 표현을 동일하게 했다. 모두 전통별 정식 공개 전이며, ONARIA의 성경 기반 테스트와 구분한다.
...
- `build/onaria-web-results`: 화면별 전체 스크린샷 및 390/768/1440 Hero/How/Paths/ONARIA/Safety 스크린샷. 요소 경계 검사와 사람이 보는 새 디자인 기준 이미지다. OS별 글꼴이 달라 pixel 동일성 검사를 했다고 주장하지 않는다.
